...
# Use python-telegram-bot v20.x with async
...
async def check_multiple_cards(update: Update, context: CallbackContext):
    """Check multiple cards - Prompt for file upload"""
    try:
        # If user provided cards directly in command (backward compatibility)
        if context.args:
            await process_card_list(update, context.args)
        else:
            await update.message.reply_text(
...
async def handle_document(update: Update, context: CallbackContext):
    ...
    try:
        ...
        # Process the cards
        await process_card_list(update, cards)
...
🔄 **Quick Fix:** Restart the bot with /start
        ...
        if update and update.message:
            ...
            await update.callback_query.message.reply_text(error_text, parse_mode='Markdown')
    except Exception as e:
...
def main():
    ...
    try:
        ...
        application.add_handler(CommandHandler("cc", check_single_card))
        application.add_handler(CommandHandler("ml", check_multiple_cards))
